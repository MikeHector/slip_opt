function [ c, ceq, c_linear, A, b, ceqCheck, cCheck] = getSymbolicConstraints( dv, p, v )
...
%     eqCon(5,1) = Parameters.g * (Parameters.apex_height + Parameters.deltah - y(stanceEnd)) - .5 * dy(stanceEnd)^2;
    ...
    cCheck(end+1) = dx(stanceEnd) - (v.apex_velocity);
...
%     eqCon(4:6,2) = dv(4:6,stanceEnd) - dv(4:6, stanceEnd+1);
    ...
    qE = [y(end) r0(end) dx(end) dy(end) dr0(end) Tleg(end) Tankle(end)];
    q1 = [y(1) r0(1) dx(1) dy(1) dr0(1) Tleg(1) Tankle(1)];
    concatplease = qE - q1;
...
%     end
    ...
    cCheck(end+1) = dx(1) - (x(end) - x(1)) / (dv(end-1) + dv(end));
    %Concatenate
    ceq = [ceq_stance; ceq_flight; eqCon'];
    c_linear = c_linear';
    ceqCheck = [ceq_stance; ceq_flight; cCheck'];
