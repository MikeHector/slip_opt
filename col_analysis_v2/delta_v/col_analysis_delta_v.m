...
record_video = 0;
if record_video==1
    v=VideoWriter('torques_over_delta_v','MPEG-4');
    v.FrameRate=10;
    open(v);
end
...
axis([-.6,1.6, 0, 1]); xlabel('delta_v'); ylabel('cost');
...
q=1;
for k = 1:length(i)
    results_sorted_deltav{k} = results{i(k)};
    flags(k) = results{i(k)}.flag;
    if results{i(k)}.flag > 0
        deltav_graph(q) = results{i(k)}.end_vel - results{i(k)}.apex_velocity;
        cost_graph(q) = results{i(k)}.cost;
        q = q+1;
    end
end
an32.XData = deltav_graph;
an32.YData = cost_graph;
for i = 1:numel(results)
    %Make energy shared figure
    energy_leg(i) = sum(results_sorted_deltav{i}.Tleg.^2);
    energy_ankle(i) = sum(results_sorted_deltav{i}.Tankle.^2);
    
    if results_sorted_deltav{i}.flag > 0
        time = results_sorted_deltav{i}.t;
        leg_response = results_sorted_deltav{i}.Tleg;
        ankle_response = results_sorted_deltav{i}.Tankle;
        x = results_sorted_deltav{i}.x;
        y = results_sorted_deltav{i}.y;
        r = results_sorted_deltav{i}.r;
        k = results_sorted_deltav{i}.k;
        xcop = -ankle_response .* r ./(k .*(results_sorted_deltav{i}.r0 -r).* y);
...
%             pause
        ...
        an3.XData = results_sorted_deltav{i}.end_vel - results_sorted_deltav{i}.apex_velocity;
        an3.YData = results_sorted_deltav{i}.cost;
...
%Make energy shared figure
[dvUnique, indUnique] = unique(deltav);
