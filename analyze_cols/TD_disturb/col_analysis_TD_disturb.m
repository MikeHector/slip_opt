...
axis([-0.5, .7, 0, 1.2]); title('XY Trajectory'); xlabel('X Displacement'); ylabel('Y Displacement');

%Torques
subplot(2,2,2); an2 = plot(1,1); hold on; an22 = plot(2,2);
axis([-0.5, .7, -13, 13]); title('Torque Trajectory'); xlabel('X Displacement'); ylabel('Torque');
...
axis([varminplot,varmaxplot, 0,1400]); xlabel('Touchdown Disturbance Angle'); ylabel('Cost');
title1 = title('wut');
subplot(2,2,4); an4 = plot(1,1);
axis([-0.5, .7, -.12, .12]); xlabel('X Displacement'); ylabel('Center of Pressure')
