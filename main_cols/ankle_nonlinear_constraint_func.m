function [ c, ViolationMatrix ] = ankle_nonlinear_constraint_func( DecisionVars, Parameters, seed )
...
%   violation. THIS VERSION always locks the given ankle torques
    ...
    %x pos
    ViolationMatrix(4,end) = x(1) - seed(1,1);
    %x vel
    ViolationMatrix(5,end) = dx(1) - seed(4,1);
    %y pos
    ViolationMatrix(6,end) = y(1) - seed(2,1);
    %y vel
    ViolationMatrix(7,end) = dy(1) - seed(5,1);
...
%     ass = r - r0;
    ...
    %Experimental
    %y - make sure it's coming from apex to ground/ ground to apex
    A_vio(1,end+1) = dy(1);
    A_vio(2,end+1) = -dy(end);
