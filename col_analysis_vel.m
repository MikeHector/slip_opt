...
record_video = 1;
if record_video==1
    q=VideoWriter('torques_over_velocity','MPEG-4');
    q.FrameRate=10;
    open(q);
end
strucc = dir('opt_velocity_*');
...
for i = 1:length(vel_sorted) %find(vel_sorted == vel)
...
%         an2.YData = y;  
        ...
        drawnow
        title.String = ['apex velocity = ', num2str(results_sorted_vel{i}.apex_velocity)];
        pause(.05)
        if record_video==1
            F=getframe(gcf);
            writeVideo(q,F);
