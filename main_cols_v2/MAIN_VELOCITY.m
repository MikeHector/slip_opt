...
delta_v = .01;
velocity_values = linspace(1, 2.5, (2.5 -1)/delta_v + 1);
...
while velocity < 2.5
    ankles_on = 1;
    [x_opt_ankle, opt_results] = RUN_COL(opt_seed, damping, velocity, apex_height, ankles_on, velocity, 0, NaN);
...
%         save(strcat('D:\Documents\DRL\slip_opt\opt_results\damping_results\',filename),'opt_results'); %My desktop
%         save(strcat('C:\Users\Administrator\Documents\DRL\slip_opt\opt_results\velocity_results',filename),'opt_results'); %My laptop
        save(strcat('C:\Users\DRL\Documents\mike_git\slip_opt\opt_results\velocity_results',filename),'opt_results'); %DRL laptop
...
delta_v = .01;
velocity_values = linspace(1, .5, (1 -.5)/delta_v + 1);
% damping_values = [damping_values, 200:2:1500];
apex_vel = 1; apex_height = 1.1;
